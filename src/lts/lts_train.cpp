...
ABSL_FLAG(
    libpts::algorithm::lts::PruningPolicy,
    prune_policy,
    libpts::algorithm::lts::PruningPolicy::Eager,
...
public:
    LTSLearner(std::shared_ptr<ModelT> model_wrapper, int learning_batch_size, int grad_steps)
    ...
    void preprocess([[maybe_unused]] std::vector<lts::SearchInput<EnvT, ModelT>> &batch, [[maybe_unused]] bool is_train)
    {}
    ...
    void process_data(std::vector<SearchOutputT> &&search_outputs)
    {
        ...
        for (auto &&result : std::move(search_outputs)) {
            if (result.solution_found) {
                for (auto &&[s, a] : std::views::zip(result.solution_path_states, result.solution_path_actions)) {
