    ...
    void process_data(std::vector<SearchOutputT> &&search_outputs) {
        ...
        for (auto &&result : std::move(search_outputs)) {
            if (result.solution_found) {
                for (auto &&[s, c] : std::views::zip(result.solution_path_states, result.solution_path_costs)) {
                    learning_inputs.push_back(
                        LearningInputT{.observation = s.get_observation(), .target_cost_to_goal = c}
                    );
                }
            }
        }
