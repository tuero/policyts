    ...
    copy_with_prefix_rename(const OldM &oldm, NewM &newm, const std::string &new_prefix, const std::string &old_prefix)
{
    ...
    auto new_params = newm->get_named_parameters()["policy_convnet"];
    ...
    auto new_bufs = newm->get_named_buffers()["policy_convnet"];
...
int main(int argc, char **argv)
{
    ...
    int num_actions = 5;
    libpts::ObservationShape obs_shape{27, 22, 22};
    ...
    int reduce_channels = model_config_json["policy_channels"].template get<int>();
    std::vector<int> mlp_layers = model_config_json["policy_mlp_layers"].template get<std::vector<int>>();
    bool use_batchnorm = model_config_json["use_batchnorm"].template get<bool>();

    // Old model
    PolicyConvNet
        model_old(obs_shape, num_actions, resnet_channels, resnet_blocks, reduce_channels, mlp_layers, use_batchnorm);
    ...
    auto output_model_wrapper =
        std::make_shared<ModelT>(model_config_json, obs_shape, num_actions, "cpu", absl::GetFlag(FLAGS_output_dir));
    output_model_wrapper->print();

    copy_with_prefix_rename(model_old, output_model_wrapper, "mlp.head_mlp", "mlp.policy_head_mlp");
